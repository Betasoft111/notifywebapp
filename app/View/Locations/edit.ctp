...
<div class="bread-crumb">
    <ul>
        ...
        <li><img src="<?php echo HTTP_ROOT; ?>images/arrow.png" alt=""></li>
        <li><a href="<?php echo HTTP_ROOT; ?>locations/edit/<?php echo $id; ?>" class="active">Edit</a></li>
    </ul>
</div> 
        ...
        <div class="table">
          ...
          <fieldset>
           <!--  <legend><?php //echo __('Update Location'); ?></legend> -->
            ...
            <?php
            echo $this->Form->input('location_name', array('class' => 'clickable', 'value'=>@$location['location_name'],  'label' => false )); ?>
             </div>
            <?php $sizes = array(''=>'Select','manuall_address' => 'Add Address', 'beacon_location' => 'Add Beacon');
            echo $this->Form->input(
            'location_type', array('options' => $sizes, 'default' => 'Select', 'label' => 'Location Setup', 'value'=>@$location['location_type'])
            );
            ?>
            ...
            <div id="locationAddress" class="location_setup">
              ...
              <div class="form">
                ...
                <?php echo $this->Form->input('location_Setup', array('class' => 'clickable', 'value'=>@$location['location_Setup'], 'label' => false)); ?>
              </div>
               <div class="form">
                <label class="lable_item">City</label>
                <?php echo $this->Form->input('city', array('class' => 'clickable', 'value'=>@$location['city'], 'label' => false)); ?>
              </div>
              <div class="form">
                <label class="lable_item">State</label>
                <?php echo $this->Form->input('state', array('class' => 'clickable', 'value'=>@$location['state'], 'label' => false)); ?>
              </div>
            ...
            <div id="addBeacon" class="location_setup">
              ...
              <select name="data[Location][Beacon]">
                ...
                <?php foreach($beacons as $beacon) {  ?>                 
                  <option <?php if($location['beacon_id'] == $beacon['Beacon']['id'] ) {
                      echo "selected='selected'";
                     } ?>
                    value="<?php echo $beacon['Beacon']['id']; ?>">
                    <?php echo $beacon['Beacon']['name']; ?>
                  </option>
                  <?php } ?>
              </select>
            </div>
            <?php
            echo $this->Form->input('open_time', array('value'=>@$location['open_time']));
            echo $this->Form->input('close_time', array('value'=>@$location['close_time']));
            ?>
           
            <div class="form">
             <label class="lable_item">company Name</label>
            <?php
            echo $this->Form->input('company_name', array('value'=>@$location['company_name'] , 'class' => 'clickable', 'label' => false));
            ?>
            </div>
<div class="form buttonss">
   <a href="<?php echo HTTP_ROOT; ?>locations/view/<?php echo $id; ?>">Cancel</a>
<?php echo $this->Form->end(__('Submit')); ?>
  ...
  <script type="text/javascript">
    $(document).ready(function () {
      ...
      setTimeout(function () {
        $('#LocationLocationType').trigger('change');
      },100);
      // var locTypeSeleted = $('#LocationLocationType option:selected').val();
      
      // if(locTypeSeleted === 'manuall_address') {
      //   $('#locationAddress').show('slow');
      //   $('#addBeacon').hide('slow');
      // }else if($(this).val() === 'beacon_location') {
      //   $('#locationAddress').hide('slow');
