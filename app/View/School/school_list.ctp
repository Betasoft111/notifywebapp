...
<h6>
<?php if($sown=='1'){ ?>
<a href="<?php echo HTTP_ROOT."school/schoolAdd"; ?>">Add</a><a class="deletelink" href="<?php echo HTTP_ROOT."school/delete/"; ?>">|Delete</a>
...
<table class="main_table">
  <thead>
    <tr>
      <th class="checkbox"><input type="checkbox" id="checkAll"></th>
       ...
       <?php echo "<th>" . $paginator->sort('school_name', 'School Name') . "</th>"; ?>
       <?php echo "<th>" . $paginator->sort('school_code', 'School Code') . "</th>"; ?>
      <th >Teachers</th>
      <th >Students</th>
      <th >Classes</th>
      <th >Actions</th>
  ...
  <tbody>
  <?php if(!empty($schooldata)){foreach ($schooldata as $schooldatas) {?>
   <tr>
      <td class="checkbox"><input type="checkbox" class="check" value="<?php echo $schooldatas['School']['id']; ?>"></td>
      <td><a href="<?php echo HTTP_ROOT."teachers/classList/".$schooldatas['School']['id']; ?>"><?php echo $schooldatas['School']['school_name']; ?></a></td>
      <td><?php echo @$schooldatas['School']['school_code']; ?></td>
      <td><?php  $arr1=array(); $arr2=array(); if(!empty($schooldatas['Teacher'])){
           foreach ($schooldatas['Teacher'] as $classd) {
            $arr1[]=$classd['User']['id'];
           }
      ...
      <?php  $arr3=array(); $arr4=array(); if(!empty($schooldatas['Teacher'])){
           foreach ($schooldatas['Teacher'] as $classd) {
               foreach ($classd['Student'] as $studata) {
                 $arr4[]=$studata['student_email'];
               }
           }
          $arr3=array_unique($arr4);
         echo count($arr3);
         // echo "<pre>"; print_r($arr4); 
          } else { echo "0"; }?>
       </td>
      <td><?php $i='0'; if(!empty($schooldatas['Teacher'])){
           foreach ($schooldatas['Teacher'] as $classd) {
            $i++;
           }
       echo $i;
           } else { echo "0"; }?></td>
      <td class="actions2"><span><a href="<?php echo HTTP_ROOT."school/school"; ?>">View </a></span><?php if($sown=='1'){ ?><span><a href="<?php echo HTTP_ROOT."school/schoolAdd/".$schooldatas['School']['id']; ?>">/ Edit / </a></span><span><a href="<?php echo HTTP_ROOT."school/delete/".$schooldatas['School']['id']; ?>">Delete</a></span><?php }?></td>
    </tr>
  <?php }} else {?>
<tr>
