...
public function beforeFilter() {
  ...
  public function userRegistered()
  {
   ...
   // $profile_pic = '';
    ...
    $new_name = $_POST['email'] . '.png';
    $success =file_put_contents(WWW_ROOT.'bss_files/'.$new_name,$binary);
    //die;
   // echo $success;
    //$file = fopen(WWW_ROOT.'bss_files/'.$new_name, 'wb');
    // Create File
    //fwrite($file, $binary);
    //fclose($file);
    $profile_pic = $new_name;
      //if( move_uploaded_file($file, WWW_ROOT . 'bss_files/'.$new_name) )
       // {
        //  $profile_pic = $new_name;
        //}
}
      ...
      $userdata['User']['username']=trim($_POST['fullname']);
      if(!isset($_POST['id']))
      {
      ...
      if(!isset($_POST['id']))
      {
          ...
          $userdata['User']['profile_pic']=$profile_pic;
          $usdata=$this->User->save($userdata);
          $this->User->Behaviors->load('Containable');
          $data=$this->User->find('first',array('conditions'=>array('User.id'=>$usdata['User']['id']),'fields'=>array('id','username','email','school','userview','status','code','device_token','created','profile_pic','question','answer'),'contain'=>array('Role'=>array('order' => array('Role.created' => 'ASC')))));
      if(!empty($data))
      {
        $data['full_image_url'] = 'http://abdevs.com/attendance/app/webroot/bss_files/';
...
echo json_encode(array('Error'=>'All fields are required')); die;
  ...
  function sendPushNotificationToGCM() {
        ...
        if(!empty($studentids) && !empty($_POST['message']))
        {
          $y=0;
           ...
           { 
               ...
               $msgdata['Message']['image_url']=trim($_POST['image_url']);
               $this->Message->create();
      ...
      if(!empty($userids))
      {
       ...
       define("GOOGLE_API_KEY", "AIzaSyA7e1MpMHE26kylM3M2HObOwsJlWfVc_qM"); 
        ...
        // Google Cloud Messaging GCM API Key
           ...
           if(!empty($_POST['message']) && !empty($_POST['employee_emails']) && !empty($_POST['meeting_unique_code']) && !empty($_POST['employee_id']) && !empty($_POST['user_id']))
      {
        $emails=array();
        $studentids=array();
        $studentids=explode(',',  trim($_POST['employee_id']));
        $emails=explode(',', trim($_POST['employee_emails']));
        if(!empty($studentids) && !empty($_POST['message']))
        {
          $y=0;
           for ($i=0; $i <count($studentids) ; $i++) 
           { 
               $msgdata['Message']['user_id']=trim($_POST['user_id']);
               $msgdata['Message']['employee_id']=$studentids[$i];
               $msgdata['Message']['employee_email']=$emails[$i];
               $msgdata['Message']['meetingCode']=trim($_POST['meeting_unique_code']);
               $msgdata['Message']['message']=trim($_POST['message']);
               $msgdata['Message']['image_url']=trim($_POST['image_url']);
               $this->Message->create();
               if($this->Message->save($msgdata))
      ...
      if(!empty($userids))
      {
       ...
       define("GOOGLE_API_KEY", "AIzaSyA7e1MpMHE26kylM3M2HObOwsJlWfVc_qM"); 
        ...
        { 
            ...
            $fields = array(
                'registration_ids' => array($userid['Gcmreg'][$i]['reg_id']),
                'data' => array("m" => trim($_POST['message']),"meetingCode"=>trim($_POST['meeting_unique_code'])),
    ...
    public function studentmsglist()
    {
      ...
      {
            $data=$this->Message->find('all',array('conditions'=>array('Message.user_id'=>$_POST['user_id'],'Message.class_code'=>$_POST['class_code']),'fields'=>array('DISTINCT Message.created','Message.message','Message.image_url')));
           if(!empty($data))
           {
            echo json_encode(array('Message'=>'List of students message.','Data'=>$data)); die;
           }
        ...
        if(!empty($_POST['email']) && !empty($_POST['meetingCode']))
      {
        
        $data=$this->Message->find('all',array('conditions'=>array('Message.employee_email'=>$_POST['email'],'Message.meetingCode'=>$_POST['meetingCode']),'order' => array('Message.id' => 'DESC')));
           if(!empty($data))
           {
            echo json_encode(array('Message'=>'List of students message.','Data'=>$data)); die;
           }
        ...
        if(!empty($_POST['user_id']) && !empty($_POST['meetingCode']))
      {
            $data=$this->Message->find('all',array('conditions'=>array('Message.user_id'=>$_POST['user_id'],'Message.meetingCode'=>$_POST['meetingCode']),'fields'=>array('DISTINCT Message.created','Message.message','Message.image_url')));
           if(!empty($data))
           {
            echo json_encode(array('Message'=>'List of students message.','Data'=>$data)); die;
           }
  ...
  public function addClassByStudent()
   { 
      ...
      { 
      ...
      else if(!empty($_POST['meetingCode']) && !empty($_POST['employee_email'])){
         $sats=$_POST['status'];
        $this->loadModel('Employee');
        $this->loadModel('Company');
        $count=$this->Employee->find('count',array('conditions'=>array('employee_email' => $_POST['employee_email'],'meetingCode' => $_POST['meetingCode'])));
        $teacherdata=$this->Company->find('first',array('conditions'=>array('Company.meetingCode'=>trim($_POST['meetingCode'])),'recursive'=>-1));
        $tecount=$this->Company->find('count',array('conditions'=>array('Company.meetingCode'=>trim($_POST['meetingCode'])))); 
        if($tecount=='0')
        {
        echo json_encode(array('Error'=>'Class is wrong Code.')); die;
        }
       if($count =='1')
          {
          $success=$this->Employee->updateAll(array('status' => "'$sats'"), array('employee_email' => $_POST['employee_email'],'meetingCode' => $_POST['meetingCode']));    
          
             echo json_encode(array('Message'=>'Successfully added','Data'=>$teacherdata)); die;
          }
        else
        {       
              $datastu['Student']['user_id']=$teacherdata['Company']['user_id'];
              $datastu['Student']['meetingCode']=$_POST['meetingCode'];
              $datastu['Student']['employee_email']=$_POST['employee_email'];
              $datastu['Student']['status']='1';
              $datastu['Student']['company_id']=$teacherdata['Company']['id'];
              if($this->Employee->save($datastu))
              {
             echo json_encode(array('Message'=>'Successfully added','Data'=>$teacherdata)); die;
              }
              else
  ...
  public function addEventByEventee()
   { 
      if(!empty($_POST['event_code']) && !empty($_POST['eventee_email']))
      { 
        $sats=$_POST['status'];
        $this->loadModel('Eventee');
        $this->loadModel('Event');
        $count=$this->Eventee->find('count',array('conditions'=>array('eventee_email' => $_POST['eventee_email'],'eventCode' => $_POST['event_code'])));
        $eventdata=$this->Event->find('first',array('conditions'=>array('Event.eventCode'=>trim($_POST['event_code'])),'recursive'=>-1));
        $tecount=$this->Event->find('count',array('conditions'=>array('Event.eventCode'=>trim($_POST['event_code'])))); 
        if($tecount=='0')
        {
        echo json_encode(array('Error'=>'Event code is wrong.')); die;
        }
       if($count =='1')
          {
          $success=$this->Eventee->updateAll(array('status' => "'$sats'"), array('eventee_email' => $_POST['eventee_email'],'eventCode' => $_POST['event_code']));    
          
             echo json_encode(array('Message'=>'Successfully added','Data'=>$eventdata)); die;
          }
        else
        {       
              $datastu['Eventee']['user_id']=$eventdata['Event']['user_id'];
              $datastu['Eventee']['eventCode']=$_POST['event_code'];
              $datastu['Eventee']['eventee_email']=$_POST['eventee_email'];
              $datastu['Eventee']['status']='1';
              $datastu['Eventee']['event_id']=$eventdata['Event']['id'];
              if($this->Eventee->save($datastu))
              {
             echo json_encode(array('Message'=>'Successfully added','Data'=>$eventdata)); die;
              }
              else
  ...
  /* public function addCompanyByEmployee()
   { 
      ...
      { 
        ...
        $this->loadModel('Employee');
        $this->loadModel('Company');
        $count=$this->Employee->find('count',array('conditions'=>array('Employee.employee_email' => $_POST['employee_email'],'Employee.meetingCode' => $_POST['company_code'])));
      if($count=='1')
      {
         $success=$this->Employee->updateAll(array('status' => "'$sats'"), array('employee_email' => $_POST['employee_email'],'meetingCode' => $_POST['company_code']));    
      
         $teacherdata=$this->Company->find('first',array('conditions'=>array('Company.meetingCode'=>trim($_POST['company_code'])),'recursive'=>-1));         
       if(!empty($success))
          {
             echo json_encode(array('Message'=>'Successfully added','Data'=>$teacherdata)); die;
          }  
   ...
   */
    public function addCompanyByEmployee()
   {
     if(!empty($_POST['company_code']) && !empty($_POST['employee_email']))
      { 
        $sats=$_POST['status'];
        $this->loadModel('Company');
        $this->loadModel('Employee');
        $count=$this->Employee->find('count',array('conditions'=>array('employee_email' => $_POST['employee_email'],'meetingCode' => $_POST['company_code'])));
        $eventdata=$this->Company->find('first',array('conditions'=>array('Company.meetingCode'=>trim($_POST['company_code'])),'recursive'=>-1));
        $tecount=$this->Company->find('count',array('conditions'=>array('Company.meetingCode'=>trim($_POST['company_code'])))); 
         //echo $tecount; die;
       if($tecount=='0')
        {
        echo json_encode(array('Error'=>'Meeting code is wrong.')); die;
        }
       if($count =='1')
          {
          $success=$this->Employee->updateAll(array('status' => "'$sats'"), array('employee_email' => $_POST['employee_email'],'meetingCode' => $_POST['company_code']));    
        ...
        {     
              $datastu['Employee']['user_id']=$eventdata['Company']['user_id'];
              $datastu['Employee']['meetingCode']=$_POST['company_code'];
              $datastu['Employee']['employee_email']=$_POST['employee_email'];
              $datastu['Employee']['status']='1';
              $datastu['Employee']['company_id']=$eventdata['Company']['id'];
              if($this->Employee->save($datastu))
              {
             echo json_encode(array('Message'=>'Successfully added','Data'=>$eventdata)); die;
              }
...
$studata['Eventee']['parent_email']=trim($_POST['parentEmail']);
      ...
      if(!empty($_POST['image']))
      {
        ...
        $studata['Eventee']['eventee_email']=trim($_POST['eventeeEmail']);
...
public function studentAttendens()
{
   ...
   // $teacherlatlogd=$this->Attendance->find('first',array('conditions'=>array('Attendance.user_id'=>$_POST['user_id'])));
    ...
    {
    $teacher_long  = $_POST['teach_long'];
    $teacher_lat   = $_POST['teach_lat'];
         $teacherLatLongUpdate = 0;
   if( $this->Attendance->updateAll(array('Attendance.latitude' => $teacher_lat, 'Attendance.longitude' => $teacher_long ), array( 'Attendance.user_id' => $_POST['user_id'] ) ) )
   {
      $teacherLatLongUpdate = 1;
   }
            ...
            if(!empty($stulatlogd))
              {
                ...
                //echo "<pre>"; print_r($stulatlogd); echo"</pre>";

                //$distance=$this->haversineGreatCircleDistance($teachlat,$teachlong,$stulat,$stulong); 
                $distance=$this->calculateDistance($teachlat,$teachlong,$stulat,$stulong); 
                ...
                else
                {
                  ...
                  $this->StudentAttendance->id = $this->StudentAttendance->field('id', array('StudentAttendance.student_id'=>$stuarr[$i],'StudentAttendance.teacher_id'=>trim($_POST['class_id']),'StudentAttendance.user_id'=>$_POST['user_id'], 'StudentAttendance.created'=>date('Y-m-d')));
                      if ($this->StudentAttendance->id) {
                          ...
                          {
                            $this->StudentAttendance->saveField('attend', 'P');
                            //echo "<pre>"; print_r($dis.' '.$stuarr[$i].' '.'else'); echo"</pre>";
                             $j++;
                          } 
...
{
   if(!empty($_POST['user_id']) && !empty($_POST['event_code']) && !empty($_POST['eventee_id']) && !empty($_POST['type']))
   {
    $stuatt=array();
    $j=0;
    $dis='0';
...
//    $teachlong=0.0;
//    $teachlat=0.0;
//    if (isset($teacherlatlogd['Attendance']['longitude']) && isset($teacherlatlogd['Attendance']['latitude']))
//    {
//      $teachlong=$teacherlatlogd['Attendance']['longitude'];
//      $teachlat=$teacherlatlogd['Attendance']['latitude'];
//    }
   ...
   if( $this->Attendance->updateAll(array('Attendance.latitude' => $teacher_lat, 'Attendance.longitude' => $teacher_long ), array( 'Attendance.user_id' => $_POST['user_id'] ) ) )
   {
    ...
    $teachlat=$teacherlatlogd['Attendance']['latitude'];
     ...
     if($cnt2 > '0')
     {
      ...
      {  
        $this->EventeeAttendance->updateAll(
            array('EventeeAttendance.attend' => "'A'"),
            array('EventeeAttendance.event_id'=>"'".trim($_POST['event_id'])."'", 'EventeeAttendance.event_id'=>trim($_POST['event_id']),'EventeeAttendance.user_id'=>$_POST['user_id'])
        );
        if(!empty($stuarr))
        {  
          for ($i=0; $i < count($stuarr) ; $i++) 
       ...
       {  
          for ($i=0; $i < count($stuarr) ; $i++) 
        {
            ...
            if(!empty($stuid))
            {
              ...
              if($cnt>'0')
              {
                $count25=$this->EventeeAttendance->find('count', array('conditions'=>array('EventeeAttendance.eventee_id'=>$stuarr[$i],'EventeeAttendance.event_id'=>trim($_POST['event_id']),'EventeeAttendance.user_id'=>$_POST['user_id'], 'EventeeAttendance.created'=>date('Y-m-d'))));
                if($count25 =='0')
                {
                  $stuatt['EventeeAttendance']['user_id']=trim($_POST['user_id']);
                  $stuatt['EventeeAttendance']['eventee_id']=$stuarr[$i];
   ...
   if (empty($_POST['eventee_id']))
   {
    ...
    echo json_encode(array('Message'=>'Successfully attended','Data'=>null)); die;
   }
}
    public function get_user_image()
   {
      if( !empty( $_POST['user_id'] ) && is_numeric( $_POST['user_id'] ) )
      {
        ...
        $data=$this->User->find('all',
          array('conditions'=>
            array('User.id'=>trim($_POST['user_id'])),'fields'=>array('profile_pic')) );
        if( empty( $data ) )
        {
          echo json_encode(array('Error'=>'User id not found', 'status' => 0)); die;
        }
        if( empty( $data[0]['User'] ) )
        {
          echo json_encode(array('Error'=>'Image not found', 'status' => 0)); die;
        }
        $newdata = array(
          'userdata'  => $data[0]['User'],
          'full_image_url' => 'http://abdevs.com/attendance/app/webroot/bss_files/'
        );
      
      echo json_encode(array('data'=> $newdata, 'status' => 1)); die;
      }
      else
      {
        echo json_encode(array('Error'=>'Invalid Data', 'status' => 0)); die;
      }
...
public function attandenceList()
{
  if(!empty($_POST['user_id']) &&  !empty($_POST['class_id']))
  {
    ...
    else
    {
     ...
     if(!empty($_POST['user_id']) &&  !empty($_POST['company_id']))
  {
    $this->loadModel('EmployeeAttendance');
    $data=$this->EmployeeAttendance->find('all',array('conditions'=>array('EmployeeAttendance.user_id'=>trim($_POST['user_id']),'EmployeeAttendance.company_id'=>trim($_POST['company_id'])),'order'=>array('EmployeeAttendance.created'=>'DESC')));
    if(!empty($data))
    {
      echo json_encode(array('Message'=>'Data successfully retrive','Data'=>$data)); die;
    }
...
public function event_attendencelist()
{
  if(!empty($_POST['user_id']) &&  !empty($_POST['event_id']))
  {
    $this->loadModel('EventeeAttendance');
    $data=$this->EventeeAttendance->find('all',array('conditions'=>array('EventeeAttendance.user_id'=>trim($_POST['user_id']),'EventeeAttendance.event_id'=>trim($_POST['event_id'])),'order'=>array('EventeeAttendance.created'=>'DESC')));
...
public function deleteEventeeByEvent()
{
  if(!empty($_POST['event_code']) && !empty($_POST['eventee_email']))
  {
    $this->loadModel('Eventee');
    $this->loadModel('EventeeAttendance');
    if($this->Eventee->deleteAll(array('Eventee.eventee_email' => $_POST['eventee_email'], 'Eventee.eventCode' => $_POST['event_code'])))
    {
      if($this->EventeeAttendance->deleteAll(array('EventeeAttendance.eventee_id' => $_POST['eventee_id'], 'EventeeAttendance.eventCode' => $_POST['event_code'])))
      {
      echo json_encode(array('Message'=>'Successfully deleted')); die;
      }
    }
...
public function attandenceListByAttendee()
{
  if(!empty($_POST['email']) &&  !empty($_POST['event_id']))
  {
    $this->loadModel('EventeeAttendance');
                $this->loadModel('Eventee');
              $stuidata=$this->Eventee->find('first',array('conditions'=>array('Eventee.eventee_email'=>$_POST['email']),'fields'=>array('id')));
              $stuid=$stuidata['Eventee']['id'];
  $this->EventeeAttendance->unbindModel( array('belongsTo' => array('Eventee')));
$this->EventeeAttendance->bindModel(
        array('belongsTo' => array(
                'User' => array(
                    'className' => 'User',
                    'foreignKey' => 'user_id'
...
$data=$this->EventeeAttendance->find('all',array('conditions'=>array('EventeeAttendance.eventee_id'=>$stuid,'EventeeAttendance.event_id'=>trim($_POST['event_id']))));
    if(!empty($data))
    {
      echo json_encode(array('Message'=>'Data successfully retrive','Data'=>$data)); die;
    }
    else
    {
      ...
      if(!empty($_POST['email']) &&  !empty($_POST['company_id']))
  {
    $this->loadModel('EmployeeAttendance');
                $this->loadModel('Employee');
              $stuidata=$this->Employee->find('first',array('conditions'=>array('Employee.employee_email'=>$_POST['email']),'fields'=>array('id')));
              $stuid=$stuidata['Employee']['id'];
  $this->EmployeeAttendance->unbindModel( array('belongsTo' => array('Employee')));
$this->EmployeeAttendance->bindModel(
        array('belongsTo' => array(
                'User' => array(
                    'className' => 'User',
                    'foreignKey' => 'user_id'
...
$data=$this->EmployeeAttendance->find('all',array('conditions'=>array('EmployeeAttendance.employee_id'=>$stuid,'EmployeeAttendance.company_id'=>trim($_POST['company_id']))));
    if(!empty($data))
    {
      echo json_encode(array('Message'=>'Data successfully retrive','Data'=>$data)); die;
    }
...
public function addEventByEventHost()
   {
      ...
      if($count2 > 0 && !isset($_POST['id']))
      {
      echo json_encode(array('Error'=>'Event already exist')); die;
      }
      if (isset($_POST['id'])){
          $data['Event']['id']=trim($_POST['id']);
        }
...
public function addCompanyByManager()
   {
     ...
     /*  if($count > 0)
      ...
      echo json_encode(array('Error'=>'Company already exist')); die;
             ...
             $data['Company']['startDate']=date('Y-m-d',strtotime(trim($_POST['startDate'])));
             $data['Company']['endDate']=date('Y-m-d',strtotime(trim($_POST['endDate'])));
            // $data['Company']['district']=trim($_POST['district']);
             $data['Company']['companyCode']=trim($_POST['companyCode']);
             //$data['Company']['beacon_id']=trim($_POST['beacon_id']);
               $uncode=$this->gen_uuid();
              $count3=$this->Company->find('count',array('conditions'=>array('Company.meetingCode'=>$uncode)));
             while($count3>0)
             {
              $uncode=$this->gen_uuid();
              $count3=$this->Company->find('count',array('conditions'=>array('Company.meetingCode'=>$uncode)));
             ...
             $data['Company']['meetingCode']=$uncode;
...
public function addEmployee()
   ...
   if(empty($_POST['id']))
{
$count=$this->Employee->find('count',array('conditions'=>array('Employee.meetingCode'=>$_POST['companyCode'],'Employee.employee_email'=>$_POST['employeeEmail'])));
 if($count > 0)
   {
echo json_encode(array('Error'=>'Employee already added for same Company Code')); die;
   }
}  
else
{
$count=$this->Employee->find('count',array('conditions'=>array('Employee.meetingCode'=>$_POST['companyCode'],'Employee.employee_email'=>$_POST['employeeEmail'],'Employee.id !=' => $_POST['id'])));
 if($count > 0)
   {
echo json_encode(array('Error'=>'Employee Email id is already used please try another.')); die;
   }
}  
        if(!empty($_POST['id']))
         {
       $studata['Employee']['id']=trim($_POST['id']);
        ...
        $studata['Employee']['meetingCode']=trim($_POST['companyCode']);
...
$studata['Employee']['parent_email']=trim($_POST['parentEmail']);
      ...
      if(!empty($_POST['image']))
      {
       ...
       $data=$this->Employee->save($studata);
        if(!empty($data))
        {
         ...
         {
            $Email->template('welcomecompany')->emailFormat('html')->from(array('notify-noreply@abdevs.com' => 'Attendance App'))->to(array(trim($_POST['employeeEmail'])))->viewVars(array('meetingCode' => $_POST['companyCode']))->subject('For conformation')->send('you have registered for belove Meeting Code');
          echo json_encode(array('Message'=>'Saved successfully','Data'=>$data)); die;
         }
    ...
    public function deleteAttendanceByEventHost()
    {
      if(!empty($_POST['Attendance_id']))
      { 
           $this->loadModel('EventeeAttendance');
          if($this->EventeeAttendance->delete($_POST['Attendance_id']))
          {
            echo json_encode(array('Message'=>'Successfully deleted')); die;
          }
          else
    ...
    /* foreach($data as $index => $datas){
     ...
     $data2 .= $datase['Student']['student_email'].",".$datase['StudentAttendance']['created'].",".$datase['StudentAttendance']['attend']."\n";
      ...
      $this->loadModel('Employee');
      $this->loadModel('EmployeeAttendance');
      $data = $this->EmployeeAttendance->find('all',array('conditions'=>array('EmployeeAttendance.company_id'=>$_POST['company_id'])));
     //echo "<pre>"; print_r($data);die;
    /* foreach($data as $index => $datas){
                $vendorRecord = $this->Student->findById($datas['StudentAttendance']['student_id']);
                $arra1=$vendorRecord['Student']['student_email'];
     ...
     $data2 .= "Email".","."Date".","."Attendance"."\n";
     foreach ($data as $datase) {
     $data2 .= $datase['Employee']['employee_email'].",".$datase['EmployeeAttendance']['created'].",".$datase['EmployeeAttendance']['attend']."\n";
     }
      $file = 'file.csv';
      chmod($file, 0777);
      if(file_put_contents($file, $data2))
    ...
    public function eventexport() 
    {
      ...
      {
      $this->loadModel('EventeeAttendance');
      $data = $this->EventeeAttendance->find('all',array('conditions'=>array('EventeeAttendance.event_id'=>$_POST['event_id'])));
     
     $data2="";
     $data2 .= "Email".","."Date".","."Attendance"."\n";
     foreach ($data as $datase) {
     $data2 .= $datase['Eventee']['eventee_email'].",".$datase['EventeeAttendance']['created'].",".$datase['EventeeAttendance']['attend']."\n";
     }
      $file = 'file.csv';
      chmod($file, 0777);
      if(file_put_contents($file, $data2))
    ...
    public function customReport()
    {
      ...
      $data = $this->StudentAttendance->find('all',array('conditions'=>array('StudentAttendance.teacher_id'=>$_POST['class_id'],'StudentAttendance.created'=>$date2)));
        ...
        if(!empty($_POST['company_id']) && !empty($_POST['date'])){
        $this->loadModel('Employee');
        $this->loadModel('EmployeeAttendance');
        $date2=date('Y-m-d',strtotime($_POST['date']));
      $data = $this->EmployeeAttendance->find('all',array('conditions'=>array('EmployeeAttendance.company_id'=>$_POST['company_id'],'EmployeeAttendance.created'=>$date2)));
        if(!empty($data))
        {
          echo json_encode(array('Message'=>'Data successfully found','Data'=>$data)); die;
        }
    ...
    public function eventcustomReport()
    {
      if(!empty($_POST['event_id']) && !empty($_POST['date']))
      {
      $this->loadModel('EventeeAttendance');
      $date2=date('Y-m-d',strtotime($_POST['date']));
      $data = $this->EventeeAttendance->find('all',array('conditions'=>array('EventeeAttendance.event_id'=>$_POST['event_id'],'EventeeAttendance.created'=>$date2)));
        if(!empty($data))
        {
          echo json_encode(array('Message'=>'Data successfully found','Data'=>$data)); die;
        }
    ...
    public function editAttandenceByEventHost()
    {
      if(!empty($_POST['Attendance_id']) && !empty($_POST['attend']))
      {
        $this->loadModel('EventeeAttendance');
         $this->EventeeAttendance->id = $_POST['Attendance_id'];
          if($this->EventeeAttendance->saveField('attend',$_POST['attend']))
          {
             echo json_encode(array('Message'=>'Successfully updated')); die;
          }
          else
...
public function braintreestuff()
{
	//echo phpinfo();die;
  try{
  	require('/home/abdevs/public_html/attendance/app/Vendor/braintree/lib/Braintree.php');
  Braintree_Configuration::environment('sandbox');
  Braintree_Configuration::merchantId('kcy8n7w3n6n77r7z');
  Braintree_Configuration::publicKey('sbm8hh5wnk79stwk');
  Braintree_Configuration::privateKey('913a6c446c69b4b5a3bfdd9e0e2ee5b9');
     // App::import('Vendor', 'braintree','lib', array('file' => 'braintree/lib/Braintree.php'));  
  $clientToken = Braintree_ClientToken::generate();
  echo json_encode(array('Data'=>$clientToken)); die;
      //requireDependencies(); die;
  }catch(Exception $e){
